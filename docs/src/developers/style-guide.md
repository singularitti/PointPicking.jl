...
!!! info
    The style guide is always a work in progress, and not all PointPicking code
    follows the rules. When modifying PointPicking, please fix the style violations
...
PointPicking uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/PointPicking.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the PointPicking directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to PointPicking have
