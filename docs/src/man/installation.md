...
[PointPicking](https://github.com/singularitti/PointPicking.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("PointPicking")
   ```

3. Run

   ```julia-repl
   julia> using PointPicking
   ```
...
version of PointPicking, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/PointPicking.jl"
```
...
our [GitHub repository](https://github.com/singularitti/PointPicking.jl)
for new releases.
Once we release a new version, you can update PointPicking by typing

```@repl
using Pkg
Pkg.update("PointPicking")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("PointPicking")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall PointPicking.
